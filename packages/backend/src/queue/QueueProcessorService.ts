...
export class QueueProcessorService implements OnApplicationShutdown {
	...
	constructor(
		...
		private cleanRemoteNoteFilesProcessorService: CleanRemoteNoteFilesProcessorService,
		...
		//#region system
		{
			const processer = (job: Bull.Job) => {
				switch (job.name) {
					...
					case 'cleanRemoteNoteFiles': return this.cleanRemoteNoteFilesProcessorService.process();
					case 'cleanTranscodingArtifacts': return this.transcodingCleanupService.process();
		...
		//#region video transcoding
		{
			this.videoTranscodingQueueWorker = new Bull.Worker(QUEUE.VIDEO_TRANSCODING, (job) => {
				return this.telemetryService.startSpan('Queue: VideoTranscoding', () => this.videoTranscodingProcessorService.process(job));
			}, {
