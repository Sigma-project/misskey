...
export class FileServerProxyHandler {
	...
	private async processImage(
		...
		if ('static' in query) {
			return this.imageProcessingService.convertSharpToJxlStream(await sharpBmp(file.path, file.mime), 498, 422);
		}

		if ('preview' in query) {
			return this.imageProcessingService.convertSharpToJxlStream(await sharpBmp(file.path, file.mime), 200, 200);
		}
		...
		if (file.mime === 'image/svg+xml') {
			return this.imageProcessingService.convertToJxlStream(file.path, 2048, 2048);
		}
	...
	private async processEmojiOrAvatar(
		...
		const isStatic = 'static' in query;
		...
		if (isAnimated) {
			...
			const height = 'emoji' in query ? 256 : 640;
			// width は実質制限なし (height のみで制約)
			return await this.wasmVipsService.convertAnimatedToJxl(inputBuffer, 16383, height, jxlDefault);
		}

		return {
			data: sharpInstance
				.resize({
					height: 'emoji' in query ? 256 : 640,
