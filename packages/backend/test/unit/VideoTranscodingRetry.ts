...
import RetryJob from '@/server/api/endpoints/admin/video-transcoding/retry-job.js';

vi.mock('@/core/QueueService.js', () => ({ QueueService: class {} }));
...
function harness(remove?: () => Promise<void>) {
	...
	const repository = {
		findOneBy: vi.fn(async () => file),
		update: vi.fn(async () => {}),
	};
	const queue = {
		videoTranscodingQueue: { getJob: vi.fn(async () => remove ? { remove } : null) },
		createVideoTranscodingJob: vi.fn(async () => {}),
	};
