...
### General
- Enhance: upstream 2026.9.0 を取り込み、fork の JXL 変換と動画再生を維持

### Client
- Fix: チュートリアルの欠落していたJXL画像を追加し、正しいMIME型で配信
- Fix: 誕生日ウィジェットの期間・時差・夏時間による日付表示のずれとログアウト時のエラーを修正
- Fix: 新しいライトボックスでも変換済み動画のHLS再生と元動画へのフォールバックを維持

### Server
- Fix: アニメ画像のJXL変換ワーカーを未使用時と一定の処理件数ごとに自動終了し、保持されるメモリを解放
- Fix: リモート画像から登録・更新する絵文字を専用コピーで保存し、既存絵文字の画像をキャッシュ回収から保護
- Fix: 動画変換の開始・スキップ処理によるキャンセルの上書きと、完了・スキップ後の誤った失敗通知を修正
- Fix: 動画情報の取得がタイムアウトした際にffprobeを終了し、出力量にも上限を設定
- Fix: フォロワー限定ノートの返信先・メンション先にもリアクションや投票・削除の更新が届くように修正
- Fix: 動画の変換完了とファイル削除の競合による成果物の残存を防ぎ、回収失敗後も再試行するよう修正
- Fix: リモート添付ファイル回収のS3通信を期限付きで中断し、削除の再試行と他の定期処理の継続を可能に
- Enhance: 既存のリモート投稿清掃で削除した投稿の未使用添付ファイルを、共有参照を保護して再試行可能な方式で回収
- Fix: DockerイメージにJXL対応libvipsを組み込み、画像変換を検証
- Fix: 動画変換の保存失敗・VVC 失敗時の残骸、長さ不明時の早期タイムアウト、キャンセル後の進捗表示を修正
- Fix: ファイルが見つからない場合の代替画像が正しく配信されない問題を修正
- Fix: sharp 更新後も JXL 対応 libvips に対してソースビルドされるよう修正
- Fix: フォロー中ユーザーの誕生日取得で年越しの順序と利用者の暦年、平年の閏日を正しく扱うように
