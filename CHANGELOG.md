...
- オリジナル画像を保持せずにアップロードする場合 JPEG XL でアップロードされるように(Safari以外)
...
- OpenAPIエンドポイントを復旧
- WebP/AVIF/JPEGのweb公開用画像は、サーバーサイドではJPEGではなく JPEG XL に変換するように
...
- API: カスタム絵文字エンティティに`url`プロパティが含まれなくなりました
  - 絵文字画像を表示するには、`<instance host>/emoji/<emoji name>.jxl`にリクエストすると画像が返ります。
  - e.g. `https://p1.a9z.dev/emoji/misskey.jxl`
  - remote: `https://p1.a9z.dev/emoji/syuilo_birth_present@mk.f72u.net.jxl`
