...
description: Use at every "finish" moment of a Misskey change — immediately before committing, opening a PR, merging, or handing the work back to the user even without a commit. Runs the final pre-ship checklist — `pnpm lint`, misskey-js regeneration (`pnpm build-misskey-js-with-types`) when backend API changed, `pnpm --filter backend check-migrations` when entities or migrations changed, SPDX header verification on new files, locale safety check (no edits to locale yml files other than `ja-JP` / `en-US`), fork-invariant check (JXL pipeline, wasm-vips external, mise CI), and `CHANGELOG.md` Unreleased entry for user-visible changes. Must be consulted as the last step of every change — including uncommitted handoffs — to avoid CI failures and lost translations. This is NOT waived by having already invoked brainstorming, writing-plans, or any other upstream skill — invoke this regardless of what preceded it.
---
...
- [ ] lint が通る — ECC 由来の [/quality-gate](../../commands/quality-gate.md) コマンドで lint (typecheck + eslint) + 高速テストをまとめて回すのが基本。lint だけ単発で確認したいなら `pnpm lint` 直接でもよい (mise がシェルに通っていなければ `mise exec -- pnpm lint`)。**fork 既知の例外**: frontend-builder の typecheck は `@oxc-project/types` の二重バージョンで upstream 由来の失敗をする (upstream CI は frontend-builder を typecheck しない)。他 workspace が通っていれば lint 通過とみなす
...
- [ ] エンティティ (`packages/backend/src/models/*.ts` の `@Column` / `@Entity` / `@Index`) を変更した → `pnpm --filter backend check-migrations` が pending DDL 0 件で通る (前提: `docker compose -f compose.test.yml up -d --wait` で DB を起動し、`MISSKEY_CONFIG_YML=test.yml pnpm --filter backend migrate` を先に適用)
...
- [ ] `locales/` を編集した → **`ja-JP.yml` と `en-US.yml` だけ** を変更しており、他言語 yml の diff は出ていない (`git diff --name-only master -- 'locales/*.yml' | grep -vE '^locales/(ja-JP|en-US)\.yml$'` が空)。fork 独自キーは ja-JP (必須) / en-US (推奨) の両方へ
...
- [ ] fork 不変条件に抵触していない → 画像変換を WebP に戻していない / `rolldown.config.ts` の external から `wasm-vips` を外していない / CI workflow の mise + setup-libvips 構成を setup-node に戻していない (一覧は [AGENTS.md「Fork 固有の不変条件」](../../../AGENTS.md))
