...
export const DEFAULT_INTEGRATION_REFS = ['origin/master', 'master'];

/**
 * Include staged paths even when unstaged edits restore the HEAD content.
 * @param {string} base
 * @returns {string[]}
 */
export function listChangedFiles(base) {
	return [...new Set([
		...
		...gitPaths(['diff', '--cached', '--name-only', '-z', 'HEAD']),
